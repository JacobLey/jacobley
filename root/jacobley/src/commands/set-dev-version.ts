...
export const setDevVersion: CommandModule<unknown, {
    project: string;
}> = {
    command: 'set-dev-version [project]',
    describe: 'Suffix package versions with `-dev.HASH`',
    builder: yargs => yargs.positional('project', {
    ...
    handler: async options => {
        ...
        const allPackages = [
            project.packageMeta,
            ...project.dependencies.map(dependencyName => dependencyMap[dependencyName]!.packageMeta),
            ...project.devDependencies.map(devDependencyName => dependencyMap[devDependencyName]!.packageMeta),
        ];
        ...
        for (const packageMeta of allPackages) {
            ...
            todoWrites.push(async () => writeFile(
